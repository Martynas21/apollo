...
impl Live {
    ...
    fn spawn(mut self) {
        std::thread::Builder::new()
            .name("songbird-mixer".into())
            .spawn(move || {
                raise_mixer_thread_priority();
                self.run();
            })
            .expect("failed to spawn songbird mixer thread");
    }
}
...
fn raise_mixer_thread_priority() {
    unsafe {
        ...
        let param = libc::sched_param {
            sched_priority: (max_prio / 2).max(1),
        };
        let rt_ok = libc::pthread_setschedparam(libc::pthread_self(), libc::SCHED_FIFO, &param) == 0;

        if rt_ok {
            tracing::info!("songbird mixer thread: raised to SCHED_FIFO real-time priority");
            return;
        }

        *libc::__errno_location() = 0;
        let nice_result = libc::nice(-10);
        let nice_failed = nice_result == -1 && *libc::__errno_location() != 0;

        if nice_failed {
            tracing::warn!(
                "songbird mixer thread: could not raise scheduling priority (real-time \
                 and nice both denied) -- it remains vulnerable to being starved by ordinary \
                 CPU contention. Grant CAP_SYS_NICE or an rtprio limit to fix this properly."
            );
        } else {
            tracing::info!("songbird mixer thread: raised niceness (real-time scheduling unavailable)");
        }
    }
}
